...
public class BoardGui  extends JFrame  {
    ...
    public  BufferedImage  brick,image,image2,pacp1,pacp1close,pacp2,pacp2close,ghostp1,ghostp2,obstacle,food,bombLarge,bombSmall,boardBackgroundImage;
   public  BufferedImage  normalFoodImage,negitiveFoodImage;
 ...
 // int obstacleArray[][];
  ...
  private void initGraphicsImg(){
      ...
      try {
            ...
            obstacle=ImageIO.read(new File(imageDir+"carpet.jpg"));
            brick=ImageIO.read(new File(imageDir+"brick.jpg"));
            ...
            negitiveFoodImage=ImageIO.read(new File(imageDir+"negitiveFood.png"));
...
/*
  public static void main(String args[]) throws Exception {
		new BoardGui(new BoardStructure(),controller);

  }*/

  void showFrame(){
    Dimension frameSize=new Dimension(2000,2000);
...
//drawclass is a internal class of BoardGui class
	 ...
	 class DrawLabel extends JLabel {
	 	...
	 	Color boardColor=new Color(50,50,50);
                ...
                Color specialPowerCol= Color.blue;
	 	...
	 	layer_gui_data BoardData=new layer_gui_data("Battle field",1,gridLength,cellSize,100,50,planeGap);
		...
		g.setFont(new Font("TimesRoman", Font.ITALIC, fontSize));
                 ...
                 g.setColor(Color.WHITE);
                   g.drawString("Player0: "+boardStructure.score[0]+"        \nPlayer1: "+boardStructure.score[1]+"        \nRemaining Move: "+(constant.totalMove-1-Controller.totalMove), TOP+20, TOP+fontSize);
             }
	    ...
	    void drawObstacle(int layerInadex ,Graphics g){//find and save all output of cell of a layer
                ...
                for(int i=0;i<BoardData.cellPerPlane;i++){
                        for(int j=0;j<BoardData.cellPerPlane;j++){
                                if(boardStructure.grid[i][j].obstacle==1){
                                    ...
                                    cellOrigin.x=BoardData.originX+BoardData.cellSize*(i);
                                    cellOrigin.y=BoardData.originY+BoardData.cellSize*(j);
                                    g.drawImage(brick, cellOrigin.x+1,cellOrigin.y+1, this);
	  	...
	  	void fill_cell(int x,int y,Color color,Graphics g){
	  		...
	  		g.fillRect(cellOrigin.x+1,cellOrigin.y+1,BoardData.cellSize-1,BoardData.cellSize-1);
	  		repaint();
	  	}
